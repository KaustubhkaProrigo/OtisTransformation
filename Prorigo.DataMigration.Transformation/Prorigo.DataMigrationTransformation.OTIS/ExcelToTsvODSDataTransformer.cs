...
namespace Prorigo.DataMigrationTransformation.OTIS
    {
        public class ExcelToTsvODSDataTransformer : IDataTransformer
        {
            private readonly IConfiguration _configuration;
            private readonly IServiceProvider _serviceProvider;
            private readonly ILogger<ExcelToTsvODSDataTransformer> _logger;
            ...
            public ExcelToTsvODSDataTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvODSDataTransformer> logger)
            {
                _configuration = configuration;
                _serviceProvider = serviceProvider;
                _logger = logger;

                var Configuration = _configuration.GetSection("ExcelToTsvODSData");
            ...
            public void Transform(string LicenseKey)
            {
                ...
                if (isLicenValid)
                {
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "ODS"));
            }
                else
                {
            ...
            private void TransformFile(string fileNameWithPath)
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(fileNameWithPath)))
                {
                    ...
                    {
                        if (worksheet.Name.Contains("ODS Data") || worksheet.Name.Contains("ModuleGroupODSRel"))
                        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "ODS"), _objectCountPerFile)
            {
