...
{
    public class ExcelToTsvUDCTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvUDCTransformer> _logger;
        ...
        public ExcelToTsvUDCTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvUDCTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvUDC");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "UDC"));
            }
            else
            {
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "UDC"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_UDCTemplate",
                TypeName = "Conv_ExcelToTSV_UDCTemplate",
                FileExtension = "tsv"
            };
