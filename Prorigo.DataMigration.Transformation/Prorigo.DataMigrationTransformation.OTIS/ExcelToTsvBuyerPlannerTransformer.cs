...
{
    public class ExcelToTsvBuyerPlannerTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvBuyerPlannerTransformer> _logger;
        ...
        public ExcelToTsvBuyerPlannerTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvBuyerPlannerTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvBuyerPlanner");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "BuyerPlanner"));
            }
            else
            {
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "BuyerPlanner"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_BuyerPlannerTemplate",
                TypeName = "Conv_ExcelToTSV_BuyerPlannerTemplate",
                FileExtension = "tsv"
            };
