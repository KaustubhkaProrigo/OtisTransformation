...
{
    internal class PMSCalToInputTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<PMSCalToInputTransformer> _logger;
        ...
        public PMSCalToInputTransformer(IConfiguration configuration, ILogger<PMSCalToInputTransformer> logger, IMigrationDiagnostics migrationDiagnostics)
        {
            _configuration = configuration;
            _logger = logger;
            _migrationDiagnostics = migrationDiagnostics;

            var CalculationSheetToInputSection = _configuration.GetSection("PMSCalToInput");
        ...
        {
            var CalculationSheetToInputWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "PMS"), _objectCountPerFile)
            {
                FileBaseName = "TRCalculationSheetToInputRelationship",
                TypeName = "CalculationSheetToInput",
                FileExtension = "tsv",
            };

            var MissingRecordWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "PMS", "CalculationSheetToInput"), _objectCountPerFile)
            {
                FileBaseName = "TRCalculationSheetToInputRelationship",
                TypeName = "MissingCalculationSheetToInput",
                FileExtension = "tsv",
            };
            ...
            var InputReader = new TypeDataFileReader(Path.Combine(_processAreaDataPath, "BOM Files", "PMS"));
            var InputEntities = InputReader.ReadAllEntities<PMS_SCSInputEntity>("Input", "*.tsv"); //Read Input Entities
                ...
                using (MissingRecordWriter)
                {
                    foreach (var InputEntity in InputEntities)
                    {
                        if (CalculationSheetToInputWriter.HeaderRow == null)
                            CalculationSheetToInputWriter.HeaderRow = "ConnectionId\tCONFIG_ID\tFromFilename\tToFilename\tSourceID\tRelatedId\tCREATED_ON\tCREATED_BY_ID\tMODIFIED_ON\tMODIFIED_BY_ID\tPERMISSION_ID\tIS_CURRENT\tIS_RELEASED\tMAJOR_REV\tNOT_LOCKABLE\tGENERATION\tSTATE\tValueList\tSort_Order\n";

                        if (MissingRecordWriter.HeaderRow == null)
                            MissingRecordWriter.HeaderRow = "PMSNO\tParameter\tError Description\n";

                        if (CalculationSheetEntityMap.ContainsKey(InputEntity.CalSheetNo) && (ParameterEntityMap.ContainsKey(InputEntity.ParameterName)))
                        {
                            var ConnectionId = TransformerUtils.GetNewArasGuid();
                            var CONFIG_ID = ConnectionId;
                            var FromFilename = InputEntity.CalSheetNo;
                            var ToFilename = InputEntity.ParameterName;
                            var SourceID = CalculationSheetEntityMap[InputEntity.CalSheetNo];
                            var RelatedId = ParameterEntityMap[InputEntity.ParameterName];
                            ...
                            CalculationSheetToInputWriter.WriteRow($"{ConnectionId}\t{CONFIG_ID}\t{FromFilename}\t{ToFilename}\t{SourceID}\t{RelatedId}\t{CREATED_ON}\t{CREATED_BY_ID}\t{MODIFIED_ON}\t{MODIFIED_BY_ID}\t{PERMISSION_ID}\t{IS_CURRENT}\t{IS_RELEASED}\t{MAJOR_REV}\t{NOT_LOCKABLE}\t{GENERATION}\t{STATE}\t{InputEntity.ValueListorValueRange}\t{InputEntity.ID}\n");
                            successCount++;
                        }
                        else
                        {
                            var FromFilename = InputEntity.CalSheetNo;
                            var ToFilename = InputEntity.ParameterName;
                            if (!CalculationSheetEntityMap.ContainsKey(InputEntity.CalSheetNo))
                            {
                                if (!ParameterEntityMap.ContainsKey(InputEntity.ParameterName))
                                {
                                    MissingRecordWriter.WriteRow($"{FromFilename}\t{ToFilename}\tMissing PMS And Parameter\n");
                                    successCount++;
                                }
                                else
                                {
                                    MissingRecordWriter.WriteRow($"{FromFilename}\t{ToFilename}\tMissing PMS\n");
                                    successCount++;
                                }
                            }
                            else
