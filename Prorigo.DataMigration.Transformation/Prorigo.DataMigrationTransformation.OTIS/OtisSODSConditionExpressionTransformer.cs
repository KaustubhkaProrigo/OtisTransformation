...
{
    internal class OtisSODSConditionExpressionTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<OtisSODSConditionExpressionTransformer> _logger;
        ...
        public OtisSODSConditionExpressionTransformer(IConfiguration configuration, ILogger<OtisSODSConditionExpressionTransformer> logger, IMigrationDiagnostics migrationDiagnostics)
        {
            _configuration = configuration;
            _logger = logger;
            _migrationDiagnostics = migrationDiagnostics;

            var ConditionExpressionSection = _configuration.GetSection("OtisSODSConditionExpression");
        ...
        {
            var ConditionExpressionWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath,"SODS"), _objectCountPerFile)
            {
                FileBaseName = $"TR_SODSConditionExpression",
                TypeName = $"SODSCS_OutputConditionExpression",
                FileExtension = "tsv",
                HeaderRow = "SODSNo\tParameterName\tConditionExpression\tConditionTable\tisExpression\n"
            };

            var FormulaLinkupReader = new TypeDataFileReader(Path.Combine(_processAreaDataPath,"BOM Files", "SODS"));
            var FormulaLinkupEntities = FormulaLinkupReader.ReadAllEntities<SODSOutputEntity>("SODSOutput", "*.tsv");
            ...
            using (ConditionExpressionWriter)
            {
                ...
                var FormulaEntities = new List<SODSOutputEntity>();
                GroupEntity(FormulaLinkupEntities, FormulaEntities);

                void GroupEntity(IEnumerable<SODSOutputEntity> FormulaLinkupEntities, List<SODSOutputEntity> FormulaEntities)
                {
                    ...
                    foreach (var EntityGroup in EntityGroups)
                    {
                        ...
                        if (EntityGroup.Count() == 1)
                        {
                            ...
                            foreach (var entity in entities)
                            {
                                ...
                                SODSNo = entity.SODSNO;
                                ParameterName = entity.ParameterName;
                                var isExpression = "1";
                                ConditionExpressionWriter.WriteRow($"{SODSNo}\t{ParameterName}\t{entity.DefaultValueFormulaDetails}\t{ConditionTable}\t{isExpression}\n");
                            }
                        }
                        else
                        {
                            ...
                            foreach (var entity in entities)
                            {
                                if (headerIndexDict.Count() == 0)
                                {
                                    ...
                                    var splitheaders = entity.ConditionFormulaDetail.Split('|');
                                    int index = 0;
                                ...
                                {
                                    var Formula = entity.DefaultValueFormulaDetails;
                                    SODSNo = entity.SODSNO;
                                    ParameterName = entity.ParameterName;

                                    var splitConditions = entity.ConditionFormulaDetail.Split('|');
                            ...
                            ConditionExpressionWriter.WriteRow($"{SODSNo}\t{ParameterName}\t{ConditionExpression}\t[{finalOutput.TrimEnd(',')}]\t{"0"}\n");
