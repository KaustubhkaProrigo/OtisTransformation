...
{
    public class OtisPartDataTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly ILogger<OtisPartDataTransformer> _logger;
        ...
        public OtisPartDataTransformer(IConfiguration configuration, ILogger<OtisPartDataTransformer> logger, IMigrationDiagnostics migrationDiagnostics)
        {
            _configuration = configuration;
            _logger = logger;
            _migrationDiagnostics = migrationDiagnostics;

            var CADValidationSection = _configuration.GetSection("OtisPartData");
        ...
        public void DefaultValueAdder(string transformName)
        {
            ...
            using (OtisPartWriter)
            {
                foreach (var OtisGroup in OtisGroups)
                {
                    ...
                    {
                        OtisPartWriter.HeaderRow = "ARAS_UNIQUENESS_HELPER\tid\tconfig_id\tkeyed_name\titem_number\tname\tdescription\tclassification\tpart_type\tsub_type\tots_uom\teffectivity_date\tcreated_by_id\tcreated_on\tcurrent_state\tgeneration\tis_current\tis_released\tmajor_rev\tminor_rev\tmodified_by_id\tmodified_on\tpermission_id\tMAKE_BUY\tstate\tots_revision\tServiceable\tKPC_Tracking\tots_assembly_mode\tunit\tots_material\tweight\tots_us_jurisdiction\tots_us_eccn\tots_us_category\tots_us_rationale\tots_us_source\tots_us_classifieremail\tots_us_date_classified\tots_chinese_description\tots_french_description\tots_japanese_description\n";
                    }
                    ...
                    firstEntity.MAJOR_REV = string.IsNullOrEmpty(firstEntity.Revision) ? "A" : firstEntity.Revision;
                    firstEntity.MINOR_REV = "1";
                    firstEntity.MODIFIED_BY_ID = "Data Migration";
                    ...
                    firstEntity.OTS_Revision = firstEntity.MAJOR_REV + ".1";
