...
namespace Prorigo.DataMigrationTransformation.OTIS.Entities
{
    class OTSPartEntity : IReadableEntity, IWritableEntity
    {
        ...
        public string KPC_Tracking { get; set; }
        ...
        public string DataRow
        {
            get
            {
                ...
                return $"{ARAS_UNIQUENESS_HELPER}\t{ID}\t{CONFIG_ID}\t{KEYED_NAME}\t{Part_Number}\t{Part_Name}\t{Description}\t{Classification}\t{Part_Type}\t{Sub_Type}\t{UOM}\t{Effectivity_Date}\t{CREATED_BY_ID}\t{CREATED_ON}\t{CURRENT_STATE}\t{GENERATION}\t{IS_CURRENT}\t{IS_RELEASED}\t{MAJOR_REV}\t{MINOR_REV}\t{MODIFIED_BY_ID}\t{MODIFIED_ON}\t{PERMISSION_ID}\t{Make_Buy}\t{STATE}\t{OTS_Revision}\t{Serviceable}\t{KPC_Tracking}\t{Assembly_Mode}\t{Unit}\t{Material}\t{Weight}\t{U_S_Jurisdiction}\t{U_S_ECCN}\t{U_S_Category}\t{U_S_Rationale}\t{U_S_Source}\t{U_S_Classifiers_Email}\t{U_S_Date_Classified}\t{Chinese_Description}\t{French_Description}\t{Japanese_Description}\n";
        ...
        public void SetProperties(string dataRow)
        {
            ...
            var ServiceableIndex = dataRow.IndexOf('\t', RevisionIndex + 1);
            Serviceable = dataRow.Substring(RevisionIndex + 1, ServiceableIndex - RevisionIndex - 1);

            var KPC_TrackingIndex = dataRow.IndexOf('\t', ServiceableIndex + 1);
            KPC_Tracking = dataRow.Substring(ServiceableIndex + 1, KPC_TrackingIndex - ServiceableIndex - 1);

            var CREATED_BY_IDIndex = dataRow.IndexOf('\t', KPC_TrackingIndex + 1);
            CREATED_BY_ID = dataRow.Substring(KPC_TrackingIndex + 1, CREATED_BY_IDIndex - KPC_TrackingIndex - 1);
            ...
            var U_S_JurisdictionIndex = dataRow.IndexOf('\t', MaterialIndex + 1);
            U_S_Jurisdiction = dataRow.Substring(MaterialIndex + 1, U_S_JurisdictionIndex - MaterialIndex - 1);
