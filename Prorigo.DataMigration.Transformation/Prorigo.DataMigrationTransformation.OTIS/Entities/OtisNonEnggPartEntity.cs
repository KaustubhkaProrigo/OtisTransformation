...
{
    class OtisNonEnggPartEntity : IReadableEntity, IWritableEntity
    {
        ...
        public void SetProperties(string dataRow)
        {
            ...
            //var RevisionIndex = dataRow.IndexOf('\t', UOMIndex + 1);
            //Revision = dataRow.Substring(UOMIndex + 1, RevisionIndex - UOMIndex - 1);
            ...
            var ThicknessIndex = dataRow.IndexOf('\t', CREATED_ONIndex + 1);
            Thickness = dataRow.Substring(CREATED_ONIndex + 1, ThicknessIndex - CREATED_ONIndex - 1);

            var LengthIndex = dataRow.IndexOf('\t', ThicknessIndex + 1);
            Length = dataRow.Substring(ThicknessIndex + 1, LengthIndex - ThicknessIndex - 1);

            var WidthIndex = dataRow.IndexOf('\t', LengthIndex + 1);
            Width = dataRow.Substring(LengthIndex + 1, WidthIndex - LengthIndex - 1);

            var HeightIndex = dataRow.IndexOf('\t', WidthIndex + 1);
            Height = dataRow.Substring(WidthIndex + 1, HeightIndex - WidthIndex - 1);

            var Branch_PlantIndex = dataRow.IndexOf('\t', HeightIndex + 1);
            Branch_Plant = dataRow.Substring(HeightIndex + 1, Branch_PlantIndex - HeightIndex - 1);

            var LocationIndex = dataRow.IndexOf('\t', Branch_PlantIndex + 1);
            Location = dataRow.Substring(Branch_PlantIndex + 1, LocationIndex - Branch_PlantIndex - 1);

            var MODIFIED_BY_IDIndex = dataRow.IndexOf('\t', LocationIndex + 1);
            MODIFIED_BY_ID = dataRow.Substring(LocationIndex + 1, MODIFIED_BY_IDIndex - LocationIndex - 1);
            ...
            var GradeIndex = dataRow.IndexOf('\t', MaterialIndex + 1);
            Grade = dataRow.Substring(MaterialIndex + 1, GradeIndex - MaterialIndex - 1);

            var ServiceableIndex = dataRow.IndexOf('\t', GradeIndex + 1);
            Serviceable = dataRow.Substring(GradeIndex + 1, ServiceableIndex - GradeIndex - 1);
