...
namespace Prorigo.Plm.DataMigration.OtisDataTransformer
{
    public class OtisParameterFeatureOptionTransformer : IDataTransformer
    {
        ...
        public void DefaultValueAdder(string transformName, string classification)
        {
            ...
            var OtisEntityReader = new TypeDataFileReader(Path.Combine(_processAreaDataPath,"Parameter"));
            ...
            HashSet<(string source_id, string related_id)> uniqueFeatureOptionRelationship = new HashSet<(string, string)>();
            long successCount = 0;
                                    ...
                                    foreach (var OtisGroup in OtisGroups)
                                    {
                                        ...
                                        foreach (var options in OtisGroup)
                                        {
                                            ...
                                            if (!uniqueVMOptions.Contains(options.Value))
                                            {
                                                ...
                                                var item_number = $"OPT-{vmOptionSequence.ToString("D6")}";
                                        ...
                                        foreach (var featureoptions in OtisGroup)
                                        {
                                            ...
                                            if (string.IsNullOrWhiteSpace(source_id) || string.IsNullOrWhiteSpace(related_id))
                                            {
                                                continue;
                                            }

                                            if (uniqueFeatureOptionRelationship.Contains((source_id, related_id)))
                                            {
                                                continue;
                                            }
                                            uniqueFeatureOptionRelationship.Add((source_id, related_id));
