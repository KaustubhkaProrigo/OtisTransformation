...
namespace Prorigo.DataMigrationTransformation.OTIS
    {
        public class ExcelToTsvProductTransformer : IDataTransformer
        {
            private readonly IConfiguration _configuration;
            private readonly IServiceProvider _serviceProvider;
            private readonly ILogger<ExcelToTsvProductTransformer> _logger;
            ...
            public ExcelToTsvProductTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvProductTransformer> logger)
            {
                _configuration = configuration;
                _serviceProvider = serviceProvider;
                _logger = logger;

                var Configuration = _configuration.GetSection("ExcelToTsvProduct");
            ...
            public void Transform(string LicenseKey)
            {
                ...
                if (isLicenValid)
                {
                    TransformExcelFiles(Path.Combine(_processAreaDataPath, "Product"));
                }
                else
                {
            ...
            private void TransformFile(string fileNameWithPath)
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(fileNameWithPath)))
                {
                    ...
                    {
                        if (worksheet.Name.Contains("ProductData") || worksheet.Name.Contains("PlatformProductRel"))
                        {
                            if (worksheet != null)
                            {
            ...
            private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows, string worksheetName)
            {
                ...
                var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "Product"), _objectCountPerFile)
                {
                    FileBaseName = $"Conv_ExcelToTsv_ODSTemplate_{worksheetName}",
                    TypeName = $"Conv_ExcelToTsv_ODSTemplate_{worksheetName}",
                    FileExtension = "tsv"
                };
