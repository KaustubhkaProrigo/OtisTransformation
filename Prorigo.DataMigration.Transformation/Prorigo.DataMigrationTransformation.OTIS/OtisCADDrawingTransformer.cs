...
namespace Prorigo.DataMigrationTransformation.OTIS
{
    public class OtisCADDrawingTransformer : IDataTransformer
    {
        ...
        public void DefaultValueAdder(string transformName)
        {
            ...
            var MissingDrawingToPartRelWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "Drawing", "DrawingToPart"), _objectCountPerFile)
            {
                FileBaseName = $"Otis_MissingDrawingToPart",
                TypeName = "MissingDrawingToPart",
                FileExtension = "tsv",
            };
            ...
            //{
            using (MissingDrawingToPartRelWriter)
            {
                using (OtisCadToPartRelWriter)
                {
                    using (OtisDrawingWriter)
                    {
                        foreach (var OtisGroup in OtisGroups)
                        {
                            ...
                            firstEntity.PERMISSION_ID = "BADFEE81D2844845BDDAEA8CB663281E";
                    ...
                    foreach (var rel in OtisDrawingToPartEntities)
                    {
                        ...
                        if (MissingDrawingToPartRelWriter.HeaderRow == null)
                            MissingDrawingToPartRelWriter.HeaderRow = "Source_DrawingNumber\tRelated_PartNumber\tErrorDescription\n";
                        ...
                        else if (!DrawingMap.ContainsKey(rel.Drawing_Number) && !partDict.ContainsKey(rel.Part_Number))
                        {
                            var Source_DrawingNumber = rel.Drawing_Number;
                            var Related_PartNumber = rel.Part_Number;
                            var MissingReason = "Both Source_DrawingNumber and Related_PartNumber are Missing";

                            MissingDrawingToPartRelWriter.WriteRow($"{Source_DrawingNumber}\t{Related_PartNumber}\t{MissingReason}\n");
                            successCount++;
                        }
                        else if (!DrawingMap.ContainsKey(rel.Drawing_Number) && partDict.ContainsKey(rel.Part_Number))
                        {
                            var Source_DrawingNumber = rel.Drawing_Number;
                            var Related_PartNumber = rel.Part_Number;
                            var MissingReason = "Source_DrawingNumber is Missing";
                        ...
                        else if (DrawingMap.ContainsKey(rel.Drawing_Number) && !partDict.ContainsKey(rel.Part_Number))
                        {
                            var Source_DrawingNumber = rel.Drawing_Number;
                            var Related_PartNumber = rel.Part_Number;
                            var MissingReason = "Related_PartNumber is Missing";
