...
{
    public class ExcelToTsvRelBranchPlantLocationTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvRelBranchPlantLocationTransformer> _logger;
        ...
        public ExcelToTsvRelBranchPlantLocationTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvRelBranchPlantLocationTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvRelBranchPlantLocation");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "BranchPlantLocation"));
            }
            else
            {
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "BranchPlantLocation"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_BranchPlantLocationTemplate",
                TypeName = "Conv_ExcelToTSV_BranchPlantLocationTemplate",
                FileExtension = "tsv"
            };
