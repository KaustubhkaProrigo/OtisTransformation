...
{
    public class ExcelToTsvInternalParameterTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvInternalParameterTransformer> _logger;
        ...
        public ExcelToTsvInternalParameterTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvInternalParameterTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvInternalParameter");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                //var path = Path.Combine(_processAreaDataPath, "ExcelDataFiles", "Internal_Output_Parameter");
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "Internal_Output_Parameter"));
            }
            else
            {
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "Internal_Output_Parameter"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_InternalParameter",
                TypeName = "Conv_ExcelToTSV_InternalParameter",
                FileExtension = "tsv"
            };
