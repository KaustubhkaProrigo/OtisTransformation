...
{
    public class ExcelToTsvBranchPlantDataTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvBranchPlantDataTransformer> _logger;
        ...
        public ExcelToTsvBranchPlantDataTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvBranchPlantDataTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvBranchPlantData");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "BranchPlant"));
            }
            else
            {
                ...
                if (worksheet != null)
                {
                    ...
                    for (int row = startRow; row <= endRow; row++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                 dataRow[i] = worksheet.Cells[row, i + 1].Value == null ? "" : worksheet.Cells[row, i + 1].Text.TrimStart().TrimEnd();
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "BranchPlant"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_BranchPlantTemplate",
                TypeName = "Conv_ExcelToTSV_BranchPlantTemplate",
                FileExtension = "tsv"
            };
