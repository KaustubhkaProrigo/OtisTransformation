...
{
    public class ExcelToTsvNonEnggPartTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvNonEnggPartTransformer> _logger;
        ...
        public ExcelToTsvNonEnggPartTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvNonEnggPartTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvNonEnggPart");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                //var path = Path.Combine(_processAreaDataPath, "ExcelDataFiles","Part");
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "Non_Engg_Parts"));
            }
            else
            {
                ...
                if (worksheet != null)
                {
                    ...
                    for (int row = startRow; row <= endRow; row++)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                dataRow[i] = worksheet.Cells[row, i + 1].Value == null ? "" : worksheet.Cells[row, i + 1].Value.ToString().TrimStart().TrimEnd();
                                //dataRow[i] = worksheet.Cells[row, i + 1].Value == null ? "" : System.Text.RegularExpressions.Regex.Replace(worksheet.Cells[row, i + 1].Value.ToString().Trim(), @"^(?:\|t\||\|n\|)+|(?:\|t\||\|n\|)+$", "");
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "Non_Engg_Parts"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_NonEnggPartTemplate",
                TypeName = "Conv_ExcelToTSV_NonEnggPartTemplate",
                FileExtension = "tsv"
            };
