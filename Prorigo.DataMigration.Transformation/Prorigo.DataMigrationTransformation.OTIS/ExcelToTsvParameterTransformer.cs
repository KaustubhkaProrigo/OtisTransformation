...
namespace Prorigo.DataMigrationTransformation.OTIS
{
    public class ExcelToTsvParameterTransformer : IDataTransformer
    {
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                //var path = Path.Combine(_processAreaDataPath, "ExcelDataFiles", "Parameter");
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "Parameter"));
            }
            else
            {
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "Parameter"), _objectCountPerFile)
            {
