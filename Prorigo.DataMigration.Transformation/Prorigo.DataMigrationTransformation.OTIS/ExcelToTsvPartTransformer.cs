...
{
    public class ExcelToTsvPartTransformer : IDataTransformer
    {
        private readonly IConfiguration _configuration;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<ExcelToTsvPartTransformer> _logger;
        ...
        public ExcelToTsvPartTransformer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<ExcelToTsvPartTransformer> logger)
        {
            _configuration = configuration;
            _serviceProvider = serviceProvider;
            _logger = logger;

            var Configuration = _configuration.GetSection("ExcelToTsvPart");
        ...
        public void Transform(string LicenseKey)
        {
            ...
            if (isLicenValid)
            {
                ...
                TransformExcelFiles(Path.Combine(_processAreaDataPath, "Part"));
            }
            else
            {
        ...
        private void GenerateTsv(string fileNameWithPath, string[] columnHeaders, List<string[]> dataRows)
        {
            ...
            var customReportDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "Part"), _objectCountPerFile)
            {
                FileBaseName = $"Conv_ExcelToTSV_PartTemplate",
                TypeName = "Conv_ExcelToTSV_PartTemplate",
                FileExtension = "tsv"
            };
