    ...
    public class ExcelEBOMInOutExtractDataTransformer : IDataTransformer
    {
        ...
        private void TransformFile(string fileNameWithPath, string processType)
        {
            ...
            using (var package = new ExcelPackage(new FileInfo(fileNameWithPath)))
            {
                ...
                {
                    string sheetName = processType == "Semi-Para (Smart)" ? "Input & Output" : "Parameter";
                    ...
                    if (worksheet.Name.Contains(sheetName))
                    {
                        ...
                        if (worksheetName != null)
                        {
                            ...
                            DrawingNo = DrawingNo.Replace("\t", "|t|").Replace("\n", "|n|").Replace("\r", "|r|").TrimEnd().TrimStart();
                            ...
                            {
                                bool shouldPersistIdAndParameter = false;
                                ...
                                else
                                {
                                    ...
                                    foreach (int colIndex in formulaValidColumnIndexes)
                                    {
                                        ...
                                        {
                                            shouldPersistIdAndParameter = true;
                                        }
                                        ...
                                        if (colIndex == 0)
                                        {
                                            ...
                                            if (!isRowEmpty && (shouldPersistIdAndParameter || !string.IsNullOrEmpty(lastId)))
                                            {
                                                cleanRow.Add(lastId);
                                            }
                                            else
                                        ...
                                        else if (colIndex == 1)
                                        {
                                            ...
                                            if (!isRowEmpty && (shouldPersistIdAndParameter || !string.IsNullOrEmpty(lastParameterName)))
                                            {
                            ...
                            GenerateTsv(processType, fileNameWithPath, inputDataRows, outputDataRows, formulaDataRows, formulaColumnHeaders, DrawingNo);
                        }
                    }
                }
            }
        }
        private void GenerateTsv(string processType, string fileNameWithPath, List<string[]> inputDataRows, List<string[]> outputDataRows, List<string[]> formulaDataRows, string[] formulaColumnHeaders, string DrawingNo)
        {
            ...
            var inputDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "InputParameters"), _objectCountPerFile)
            {
                FileBaseName = sheetTsvFileName,
                TypeName = processType,
                FileExtension = "tsv"
            };
            var outputDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "OutputParameters"), _objectCountPerFile)
            {
                FileBaseName = sheetTsvFileName,
                TypeName = processType,
                FileExtension = "tsv"
            };
            var formulaDataFileWriter = new TypeDataFileWriter(Path.Combine(_processAreaDataPath, "FormulaLinkupParameters"), _objectCountPerFile)
            {
                FileBaseName = sheetTsvFileName,
                TypeName = processType,
                FileExtension = "tsv"
            ...
            FormulaToTsv(formulaDataFileWriter, formulaDataRows, formulaColumnHeaders, DrawingNo);
        }
        ...
        private void WriteToTsv(TypeDataFileWriter fileWriter, List<string[]> dataRows)
        {
            using (fileWriter)
            {
                if (fileWriter.HeaderRow == null)
                    fileWriter.HeaderRow = "ID\tParameter_Name\tParameter_Description\tParameter_Type\tUOM\tValueList_or_ValueRange\tDrawingNo\n";
        ...
        private void FormulaToTsv(TypeDataFileWriter fileWriter, List<string[]> dataRows, string[] formulaColumnHeaders, string DrawingNo)
        {
            using (fileWriter)
            {
                ...
                if (fileWriter.HeaderRow == null)
                    fileWriter.HeaderRow = "DrawingNo\tID\tPropertyName\tFormula\tCondition\n";
